 ...
 *      hash - The sha256 hash of the password and salt
 */
function makePasswordEntry(clearTextPassword) {
	var salt = crypto.randomBytes(8).toString('hex');
	var hash = crypto.createHash('sha256').update(clearTextPassword + salt).digest('hex');
...
function doesPasswordMatch(hash, salt, clearTextPassword) {
	return crypto.createHash('sha256').update(clearTextPassword + salt).digest('hex') === hash;
}
